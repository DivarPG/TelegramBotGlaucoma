...
def invalidData(message):
    bot.reply_to(message, f'{message.from_user.first_name}, такой тип сообщения не принимается. Используйте текстовые команды для взаимодействия с ботом.')

@bot.message_handler(commands=['start'])
def startMessage(message):
    bot.send_message(message.chat.id, f'Емае, {message.from_user.first_name}... Короче надо придумать стартовое сообщение, Креативные директоры давайте.')
...
@bot.message_handler(commands=['setmedicationplan'])
def setMedicationPlanMessage(message):
    user_id = message.from_user.id
...
def process_medication_name(message):
    ...
    if message.text == 'Отмена':
        ...
        if user_id in user_temp_data:
            del user_temp_data[user_id]
        bot.send_message(message.chat.id, "Добавление препарата отменено", reply_markup=types.ReplyKeyboardRemove())
        return
...
def process_days_selection(message):
    ...
    if message.text == 'Завершить выбор дней':
        ...
        if not selected_days:
            bot.send_message(message.chat.id, "Необходимо выбрать как минимум один день")
            return
    ...
    if not day_code:
        bot.send_message(message.chat.id, "Пожалуйста, используйте кнопки для выбора дней")
        return
...
def process_description(message):
    ...
    if not temp_data:
        bot.send_message(message.chat.id, "Ошибка: данные сессии утеряны. Пожалуйста, начните процесс заново.")
        if user_id in user_states:
            del user_states[user_id]
        return
    ...
    try:
        ...
        bot.send_message(message.chat.id, "Произошла ошибка при сохранении препарата")
        return
    finally:
        conn.close()
...
@bot.message_handler(commands=['getmedicationsplan'])
def getMedicationsPlanMessage(message):
    user_id = message.from_user.id
    ...
    if not medications:
        bot.send_message(message.chat.id, "У вас нет добавленных препаратов в плане лечения.")
        return
...
@bot.message_handler(commands=['deletemedication'])
def deleteMedicationMessage(message):
    user_id = message.from_user.id
    ...
    if not medications:
        bot.send_message(message.chat.id, "У вас нет добавленных препаратов.")
        return

    user_temp_data[user_id] = {'medications': medications}
    user_states[user_id] = "SELECT_MEDICATION_TO_DELETE"

    med_list = "\n".join([f"{idx + 1}. {name}" for idx, (med_id, name) in enumerate(medications)])
    ...
    bot.send_message(
        message.chat.id,
        f"Выберите препарат для удаления:\n{med_list}\n\n"
        "Введите номер препарата:",
        reply_markup=markup
    )


@bot.message_handler(func=lambda message: user_states.get(message.from_user.id) == "SELECT_MEDICATION_TO_DELETE")
def process_medication_deletion(message):
    user_id = message.from_user.id

    if message.text == 'Отмена':
        clear_user_state(user_id)
        bot.send_message(message.chat.id, "Удаление препарата отменено", reply_markup=types.ReplyKeyboardRemove())
        return

    try:
        med_index = int(message.text) - 1
        medications = user_temp_data[user_id]['medications']

        if 0 <= med_index < len(medications):
            med_id, med_name = medications[med_index]
            ...
            bot.send_message(
                message.chat.id,
                f"Препарат {med_name} успешно удален из вашего плана лечения.",
                reply_markup=types.ReplyKeyboardRemove()
            )
        else:
            bot.send_message(message.chat.id, "Неверный номер препарата. Пожалуйста, выберите номер из списка.")
    except ValueError:
        bot.send_message(message.chat.id, "Пожалуйста, введите номер препарата цифрами.")

# Список вопросов для самодиагностики
questions = [
...
def selfDiagnosisMessage(message):
    user_id = message.from_user.id
    user_states[user_id] = {'current_question': 0, 'answers': [None] * len(questions), 'message_id': None, 'max_reached': 0}
...
def send_question(user_id, chat_id):
    ...
    if state['message_id']:
        try:
            bot.edit_message_text(chat_id=chat_id, message_id=state['message_id'], text=question_text, reply_markup=markup)
        except:
...
def callback_handler(call):
    ...
    if user_id not in user_states:
        bot.answer_callback_query(call.id, "Начните диагностику заново: /selfdiagnosis")
        return
...
def handle_other_messages(message):
    if message.text == 'Отмена':
        ...
        if user_id in user_temp_data:
            del user_temp_data[user_id]
        bot.send_message(message.chat.id, "Текущая операция отменена", reply_markup=types.ReplyKeyboardRemove())
    else:
        bot.send_message(message.chat.id, "Для взаимодействия с системой используйте соответствующие команды")
